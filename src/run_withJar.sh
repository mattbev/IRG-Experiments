...
echo './run.sh <first game> <(optional) second game> <(optional) mapping between object itypes in the two games> <(optional) number of runs to average over> <(optional) number of total episodes per run> <(optional) interval for recording (e.g., 10 means record every 10 episodes)>'
